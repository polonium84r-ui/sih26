    ...
    <div style={{ padding: '0 24px 24px 24px' }}>
      ...
      <div className="formal-panel" style={{ padding: '14px 20px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
          <strong style={{ color: 'var(--text-primary)' }}>What is Block Tetris?</strong> This Gantt grid shows railway line capacity over time. The white box represents an <strong>AI-recommended 2-hour window (01:00 - 03:00)</strong> where Engineering & Traction work together while NO passenger trains pass.
        </div>
      </div>
      ...
      <div className="formal-panel" style={{ padding: '20px', marginBottom: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div>
            <h2 style={{ fontSize: '1.15rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-primary)' }}>
              <Calendar size={18} color="var(--text-primary)" /> BLOCK TETRIS • CORRIDOR SCHEDULER
            </h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '180px repeat(9, 1fr)', gap: '4px', background: 'var(--bg-card-secondary)', border: '1px solid var(--border-strong)', padding: '12px', borderRadius: '4px 4px 0 0', fontWeight: 700, fontSize: '0.75rem', textAlign: 'center' }}>
          <div style={{ textAlign: 'left', color: 'var(--text-secondary)' }}>CORRIDOR SECTION</div>
          {hours.map((h, i) => (
            <div key={i} style={{ color: 'var(--text-primary)' }}>{h}</div>
          ))}
        </div>
        ...
        {kilometers.map((km, rIdx) => (
          <div key={rIdx} style={{ display: 'grid', gridTemplateColumns: '180px repeat(9, 1fr)', gap: '4px', background: 'var(--bg-card)', padding: '14px 12px', border: '1px solid var(--border-color)', borderTop: 'none', alignItems: 'center', position: 'relative' }}>
            <div style={{ fontWeight: 700, fontSize: '0.8rem', color: 'var(--text-primary)' }}>{km}</div>
      ...
      {selectedBlock && (
        <div className="formal-panel" style={{ padding: '20px' }}>
          <h3 style={{ fontSize: '0.95rem', fontWeight: 800, marginBottom: '12px', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', fontSize: '0.85rem' }}>
            <div style={{ background: 'var(--bg-dark)', padding: '12px', borderRadius: '4px', border: '1px solid var(--border-color)' }}>
              ...
              <p style={{ fontWeight: 700, color: 'var(--text-primary)', marginTop: '2px' }}>KM {selectedBlock.start_km} to {selectedBlock.end_km}</p>
            </div>
            <div style={{ background: 'var(--bg-dark)', padding: '12px', borderRadius: '4px', border: '1px solid var(--border-color)' }}>
              <span style={{ color: 'var(--text-tertiary)', fontSize: '0.7rem', fontWeight: 700 }}>CO-LOCATED WORK:</span>
              <p style={{ fontWeight: 700, color: 'var(--text-primary)', marginTop: '2px' }}>{selectedBlock.participating_departments.join(' + ')}</p>
            </div>
            <div style={{ background: 'var(--bg-dark)', padding: '12px', borderRadius: '4px', border: '1px solid var(--border-color)' }}>
            ...
            <div style={{ background: 'var(--bg-dark)', padding: '12px', borderRadius: '4px', border: '1px solid var(--border-color)' }}>
              ...
              <p style={{ fontWeight: 800, color: 'var(--text-primary)', marginTop: '2px' }}>{selectedBlock.optimization_score} / 100</p>
            </div>
          </div>
        </div>
      )}
